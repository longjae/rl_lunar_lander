import gymnasium as gym  # OpenAI Gym에서 환경을 로드합니다.
import collections
import random
import torch
...
class ReplayBuffer():
    ...
    def sample(self, n):
        ...
        샘플링된 transition은 미니배치로 사용되기 위해 각각의 요소를 분리하여 리스트(`s_lst`, `a_lst`, `r_lst`, `s_prime_lst`, `done_mask_lst`)에 추가합니다. 
        이 때, 'a', 'r', 'done_mask'는 리스트로 추가됩니다. 이는 PyTorch의 Tensor에 맞추기 위함입니다.

        그리고 마지막에는 분리된 리스트를 PyTorch의 Tensor로 변환하여 반환합니다. 반환되는 값은 상태(`s_lst`), 행동(`a_lst`), 보상(`r_lst`), 다음 상태(`s_prime_lst`), 종료 여부(`done_mask_lst`)를 각각 Tensor로 변환하여 반환합니다. 
...
def train(q, q_target, memory, optimizer):
    ...
    위 코드는 DQN 모델을 학습하는 train 함수입니다. 주요한 작업들을 다음과 같이 수행합니다:
        ...
        1. **미니배치 샘플링**: `memory.sample(batch_size)`를 호출하여 replay buffer로부터 크기가 `batch_size`인 미니배치를 샘플링합니다. 
        이 미니배치는 상태(`s`), 행동(`a`), 보상(`r`), 다음 상태(`s_prime`), 종료 여부(`done_mask`)를 포함합니다.

        2. **Q-value 계산**: 현재 Q-network를 사용하여 상태(`s`)를 입력으로 받아 Q-value(`q_out`)를 계산합니다. 
        이 때, 선택된 행동(`a`)에 대한 Q-value를 가져옵니다(`q_a`). 
        이는 해당 상태에서 취한 행동에 대한 예측된 Q-value입니다.

        3. **타깃 값 계산**: DQN 알고리즘에 따라 타깃 Q-value를 계산합니다. 다음 상태(`s_prime`)를 입력으로 받아서 Target Q-network를 통해 가장 큰 Q-value를 취한 후, 
        해당 값에 할인율(`gamma`)을 곱하고 종료 여부(`done_mask`)를 적용하여 타깃 Q-value(`target`)를 얻습니다.

        4. **손실 계산 및 역전파**: MSE Loss 함수를 사용하여 예측된 Q-value(`q_a`)와 타깃 Q-value(`target`) 간의 손실을 계산합니다. 
        그리고 이를 통해 네트워크를 학습하기 위해 역전파를 수행합니다.
...
def main(train_flag:bool=True):
    ...
    for n_epi in range(episode):  # 에피소드 반복
        ...
        if memory.size() > 2000:  # 버퍼 크기가 일정 이상일 때 학습
